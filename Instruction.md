...
```
Передача управления происходит по адресу 10000h, где расположен ассемблерный код в бинарном виде. Этот код создает временные GDT и PML4, 
после чего осуществляет переход в Longmode. Далее осуществляется загрузка постоянных GDT и PM4 и запуск кода ядра операционной системы.
