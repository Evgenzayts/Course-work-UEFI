...
Чтобы загрузить код в оперативную память, требуется скомпилировать его (`FASM` или `NASM`) и сформировать бинарный файл с помощью утилиты `xxd`.
_Пример:_

```C
char example[] = "\x00\x00\x00\x00\x00\x00\x00\x00"  // 0
...
**APIC** (Advanced Programmable Interrupt Controller) – улучшенный программируемый 
...
Наличие I/O APIC позволяет сбалансировано распределять прерывания от внешних устройств между процессорами. Регистры LAPIC отображаются в памяти на физической странице FEE00xxx. Этот адрес одинаков для каждого LAPIC, существующего в системе.
