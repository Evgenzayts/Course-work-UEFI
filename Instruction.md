...
Прежде, чем перейти к настройке проекта и работе с ним, необходимо установить следующие пакеты:
...
### Компиляция инструменты для сборки
...
Далее нужно отредактировать `Conf/target.txt` файл для настройки платформы сборки, целевой архитектуры, цепочки инструментов и параметров многопоточности.

#### Установка информации о сборке

В файле `Conf/target.txt` следует строки `ACTIVE_PLATFORM` и `TOOL_CHAIN_TAG`:
...
_Подсказка: если не установлена gcc5 необходимо ввести команду `sudo apt-get install gcc-5`_

Также нужно изменить `TARGET_ARCH`:

    TARGET_ARCH           = X64  
    
**Включение многопоточной сборки. Значение по умолчанию для `MAX_CONCURRENT_THREAD_NUMBER` равно 1, что отключает многопоточную сборку. Это значение должно быть изменено в зависимости от возможностей многопоточности системы. Формула такова: "1 + (2 x потоков процессора)".**  
...
В итоге получено HelloWorld UEFI приложение:
...
Чтобы запустить OVMF с QEMU, нужно иметь версию QEMU версии 0.9.1 или новее. Для установки QEMU следует ввести в консоль
...
Затем требуется скопировать файл OVMF.fd в этот каталог и преименовать OVMF.fd в bios.bin:

        bash$ cp ~/src/edk2/Build/OvmfX64/DEBUG_GCC5/FV/OVMF.fd bios.bin  
        
После чего надо создать директорию для использования в качестве образа жесткого диска для QEMU:

        bash$ mkdir hda-contents  
        
### Запуск QEMU используя OVMF

        bash$ qemu-system-x86_64 -pflash bios.bin -hda fat:rw:hda-contents -net none  
        
На экране должен появиться графический логотип и запуститься оболочка UEFI.
...
Для запуска `.efi` файла, необходимо перейти в раздел fs0 (`fs0:`). С помощью команды `ls` можно увидеть файлы в директории (_Например: HelloWorld.efi_):
...
Для удобства был создан собственный конфигурационный файл `.inf`, который должен находиться в одной папке с исполняемым С-файлом `Boot.c`. Путь к конфигурационному файлу указан в файле настроек `UEFI_projects.dsc`, путь к которому, в свою очередь, указан в `Conf/target.txt` (`ACTIVE_PLATFORM`).
...
Для получения информации о процессорах необходимо использовать протокол MP_SERVICE, функции которого находятся в библиотеке `Protocol/MpService.h`. У протокола есть зависимость `PiDxe.h`.
В конфигурационный файл `.inf` также необходимо добавить используемый протокол. 

В процессе написания кода прежде всего требуется инициализировать MpServiceProtocol с помощью функции LocateProtocol:
...
Далее необходимо получить информацию о процессорах  `EFI_PROCESSOR_INFORMATION` *`ProcessorInfo`* и вывести на экран количество процессоров, а также сведения о каждом из них:
...
Далее осуществляется загрузка постоянных GDT и PML4, затем следует запуск кода ядра операционной системы.

*Примечание:* прежде чем переводить процессор в Longmode можно проверить работу приложения из Realmode на простом ассемблерном коде, который бесконечно инкрементирует значение по определенному адресу. После запуска приложения работа инкремента проверяется командой `dmem`.
